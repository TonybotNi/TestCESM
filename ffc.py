...
if __name__ == '__main__':
    ...
    ffc = FFC_BN_ACT(in_channels=16, out_channels=16, kernel_size=3, padding=1, stride=1).to(device) 
    input = torch.rand(25, 16, 16, 16).to(device)  # 输入张量
    output = ffc(input)  # 前向传播
    fu = FourierUnit(16, 16)
    print(fu(input).shape)
    print(f"\n输入张量形状: {input.shape}")
